...
//				len = VideoReturnVal;
				if(len > 0){
					...
					if(false){
						 ...
						 int rowPadding = rowStride - pixelStride * 1920;//(mWidth)
						 Bitmap bitmap = Bitmap.createBitmap(1920/*mWidth*/+rowPadding/pixelStride, 1080/*mHeight*/, Bitmap.Config.RGB_565);
		...
		public void onReceive(Context context, Intent intent) {  
            ...
            if ("com.empia.USB_PERMISSION".equals(action)) {  
                synchronized (this) {  
                    ...
                    Log.i("bofan","UsbManager.EXTRA_DEVICE =" + intent.getParcelableExtra(UsbManager.EXTRA_DEVICE));  
                    Log.i("bofan","是否有权限了？？？？？？   " + localUsbManager.hasPermission(device));  
                    if (intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false)) {  
                        if (device != null) {  
                            // Open reader  
                        	Log.i("bofan","Opening reader: " + device.getDeviceName()  
                                    + "...");  
//                            new OpenTask().execute(device); 
                        ...
                        if (device != null) {  
                        	Log.i("bofan","Permission no EXTRA_PERMISSION_GRANTED for device "  
